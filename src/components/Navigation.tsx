  ...
  return <>
      ...
      duration: 0.6
        ...
        <div className="container max-w-6xl mx-auto px-4 flex items-center justify-between">
          <motion.a href="#" className="text-xl font-bold text-gradient" whileHover={{
          scale: 1.05
        }}>
            Petar Nikolic 
          </motion.a>
          ...
          <div className="hidden md:flex items-center gap-8">
            {navItems.map(item => <motion.button key={item.name} onClick={() => scrollToSection(item.href)} className="text-muted-foreground hover:text-foreground transition-colors text-sm tracking-wide" whileHover={{
            y: -2
          }}>
                {item.name}
              </motion.button>)}
          ...
          <button onClick={() => setIsMobileMenuOpen(!isMobileMenuOpen)} className="md:hidden p-2 text-foreground">
            <motion.div className="w-6 h-5 flex flex-col justify-between">
              <motion.span animate={isMobileMenuOpen ? {
              rotate: 45,
              y: 8
            } : {
              rotate: 0,
      ...
      <AnimatePresence>
        ...
        y: -20
            ...
            <div className="flex flex-col items-center gap-8 p-8">
              {navItems.map((item, index) => <motion.button key={item.name} onClick={() => scrollToSection(item.href)} initial={{
            opacity: 0,
            y: 20
          }} animate={{
            opacity: 1,
